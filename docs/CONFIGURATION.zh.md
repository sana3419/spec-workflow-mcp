...
[loop]
autoLoop = false         # 后台循环 runner 总开关（opt-in）
maxIterations = 50       # 循环硬上限（主安全阀）
...
| 选项 | 类型 | 取值域 | 默认值 | 描述 |
|--------|------|--------|---------|-------------|
| `default` | string | `claude`、`codex` | `claude` | 当 `tasks.md` 中某任务未标注 `_Engine` 字段时由哪个引擎处理。`claude`（主引擎）直接在宿主 Claude 会话中实现;`codex`(辅助引擎,按任务 opt-in)将任务派发给 Codex MCP 服务器。在 `tasks.md` 中给任务标注 `_Engine: codex` 即可只把该任务分担给 Codex,不受此默认值影响。 |
...
控制可选的**后台循环 runner**(`.spec-workflow/spec-loop-run.sh`):它在**另一个独立的 headless
`claude` 进程**里把某个 spec 的 Phase 4 任务跑到完成——你的交互会话因此保持空闲,随时聊天、查看进展。
...
- **生成**：`bash init.sh <project-dir>` 写入上述 `config.toml`,并把 runner 装到
  `<project-dir>/.spec-workflow/spec-loop-run.sh`。
- **启用**：在项目内把 `[loop].autoLoop` 设为 `true`(或向 `init.sh` 传 `--auto-loop`,或直接让 Claude 开)。
