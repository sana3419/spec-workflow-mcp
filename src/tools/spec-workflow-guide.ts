...
Before implementing, optionally run **harden-spec** (Specification Self-Correction): it adversarially critiques the spec for hackable ambiguity / underspecification — vague acceptance criteria, \`_Tests\` a trivial test would satisfy, missing edge/security requirements — and proposes hardening edits for you to approve. If the spec is wrong, every downstream gate faithfully verifies the wrong thing, so harden it first.
