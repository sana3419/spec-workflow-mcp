...
- Optionally add _Verify: panel_ to security-critical tasks (auth, crypto, access control). With the loop's adequacy judge enabled, a panel task is judged by a cross-family judge PLUS the security and logic reviewers (any fail reopens the task) — stronger scrutiny that the tests actually cover adversarial cases. Default (no _Verify) = a single cross-family judge. Like _Tests, _Verify is part of the spec CONTRACT — set here at spec time and reviewed at approval; the implementing agent must not add or remove it (the loop's tamper gate flags any tasks.md edit during implementation, so an implementer cannot quietly drop its own panel).
