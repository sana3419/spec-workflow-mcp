...
Claude-led collaborative development framework. Claude Code plans, implements, and reviews; OpenAI Codex is an optional engine you offload specific coding tasks to. Built on [Pimzino/spec-workflow-mcp](https://github.com/Pimzino/spec-workflow-mcp), with harness-owned verification (the [Verification Ladder](docs/VERIFICATION-LADDER.md)) and — since v3 — **Telegram control instead of a web dashboard** ([docs/TELEGRAM.md](docs/TELEGRAM.md)).
...
- **Watch progress:** Telegram `/status <spec>` (live board + pushes), `tail -f .spec-workflow/specs/<spec>/loop-run.log`, or `spec-workflow-mcp status <spec>` — all while your session stays interactive.
- **Stop:** `spec-workflow-mcp stop <spec>` or Telegram `/stop <spec>` (writes `specs/<spec>/.run/stop`; the runner exits after the current iteration and logs who asked).
- **Human gates:** `gateOnSpecGateFail` / `gateOnIntegrationFail` / `gateEveryTasks` pause the loop and post an Approve/Reject card on Telegram; decisions are HMAC-signed and stored outside the project so the implementing agent cannot self-approve. See [docs/TELEGRAM.md](docs/TELEGRAM.md).
...
## Telegram control (replaces the dashboard)

```bash
# once per machine — needs ~/.spec-workflow/telegram.env with TELEGRAM_BOT_TOKEN + TELEGRAM_ALLOW_FROM
node /path/to/spec-workflow-mcp/dist/index.js --telegram
```

One `loop_bot` daemon watches every project: a live board per running loop, pushes for blocked / tamper /
regression / judge / integration events, gate cards with **Approve / Reject** buttons, and commands
(`/status /specs /spec /tasks /task /logs /steering /start /stop /archive /cleanup …`). Spec documents are
sent as `.md` attachments — nothing long is pasted into chat. It is a separate bot from the Claude Code
Telegram *channel* plugin you may use to talk to the orchestrator session. Full guide: [docs/TELEGRAM.md](docs/TELEGRAM.md).

Human-only CLI equivalents: `spec-workflow-mcp status|stop|reset|set-status|cleanup`.
...
├── .claude/
│   ├── settings.json                  # MCP allows + deny rules protecting gates/audit/telegram
...
└── .spec-workflow/
    ...
    │       ├── verify-results/        # harness verdicts (+ judge, failureClass, manual)
    │       ├── .run/                  # pid, stop, gates/*.pending (runner-owned)
    │       └── loop-run.log           # per-spec runner log
...
| Telegram bot silent | check `~/.spec-workflow/telegram.env` (token + `TELEGRAM_ALLOW_FROM` numeric id), daemon log; a `409 Conflict` means two pollers share one token — use a dedicated loop_bot token |
