...
tags: ['observability']
triggers:
  content: ['console\.(log|error|warn)', '\blogger\.|\blog\.(info|warn|error|debug)', '\bmetrics?\b|prometheus|statsd', '\btrace|span\b|opentelemetry', 'fetch\(|axios|http\.request', '\bqueue|worker|cron\b']
---
...
Write the report to `.spec-workflow/reports/agent-observability-<YYYYMMDD-HHMMSS>.md` and print it.
