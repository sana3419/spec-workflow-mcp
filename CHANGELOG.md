...
### Fixed
- **A project created from Telegram never appeared in Telegram.** Three defects stacked: (1) `init.sh` records what it creates in `~/.spec-workflow/projects.json` while the daemon discovered projects only through the MCP `ProjectRegistry` (`~/.spec-workflow-mcp/activeProjects.json`), which is written *only* when an MCP server actually starts in a project — so a freshly created project was in neither list the daemon read; (2) `pollRequests` skipped rediscovery precisely for `new-project` completions (`r.kind !== 'new-project'`), the one kind that adds a project; (3) the `📁 New / add project` button had no "add" half — pointing it at an existing spec-workflow project filed an init request instead of adding it. Discovery now merges both sources (`mergeDiscoveredProjects`, with `ignored` honoured as a veto), every completed request triggers rediscovery, and an existing project is recorded and switched to without re-running `init.sh` over its CLAUDE.md/settings.json.
...
### Added
- **Phase 4 background loop runner (opt-in).** `init.sh` installs `.spec-workflow/spec-loop-run.sh`, a runner that drives a spec's tasks to completion in a **separate headless `claude` process** so the interactive session stays free to chat/monitor. Enable via `[loop].autoLoop = true` (or `init.sh --auto-loop`), start with `nohup bash .spec-workflow/spec-loop-run.sh <spec> &` (or just ask Claude). New `[loop]` config (`autoLoop`, `maxIterations`, `noProgressStop`); stop via `touch .spec-workflow/.loop-stop`; audit log (`.spec-workflow/loop-audit.log`). Default remains interactive/prompt-driven.
